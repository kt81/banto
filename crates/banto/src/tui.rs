...
/// `ConfirmDisband`/`ConfirmKill`/`WorkerAgentPicker`/`ConfirmCodexTrust`/
/// `ConfirmDirectorReopen` are the emporium's own modals — the chōba never
/// opens any of them (it has no brigade-formation path of its own), so
/// confirming them here is a no-op (Esc still closes it, via the shared
